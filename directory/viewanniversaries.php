...
foreach($months as $k => $v){?>
      ...
      <? 
$sql="select *, right(concat('000',substring_index(h.anniversary, '/', -1)), 2) as sortfield from dir_households as h where h.anniversary like '".$k."/%' order by sortfield, h.lastname";
$result=mysql_query($sql, $conn);
while($data = mysql_fetch_assoc($result)){
	
	$psql="select firstname from dir_people as p where p.household = ".$data["id"]." and p.role < 3 order by role";
	$presult=mysql_query($psql, $conn);
	$conj = "";
	$heads = "";
	while($pdata = mysql_fetch_assoc($presult)){
		$heads .= $conj.stripslashes($pdata["firstname"]);
		$conj = " &amp; ";
	}?>
		<tr>
		  <td width="372" valign="top"><div id="leftbox">
		  <?=stripslashes($data["anniversary"])?> - 
		  <?=stripslashes($heads." ".$data["lastname"])?>
			</div></td>
